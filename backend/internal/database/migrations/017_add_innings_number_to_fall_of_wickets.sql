...
ALTER TABLE testing_db.fall_of_wickets 
...
UPDATE testing_db.fall_of_wickets 
SET innings_number = (
    SELECT innings_number 
    FROM testing_db.innings 
    WHERE testing_db.innings.id = testing_db.fall_of_wickets.innings_id
);
...
CREATE INDEX IF NOT EXISTS idx_fall_of_wickets_innings_number ON testing_db.fall_of_wickets(innings_number);

-- Add comment for documentation
COMMENT ON COLUMN testing_db.fall_of_wickets.innings_number IS 'Innings number (1 or 2) when the wicket fell';
