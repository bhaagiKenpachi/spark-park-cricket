...
func (s *SeriesService) ListSeries(ctx context.Context, filters *models.SeriesFilters) (*interfaces.PaginatedSeriesResult, error) {
	// Set default values
	if filters.Limit <= 0 {
		filters.Limit = 20
	}
	...
	result, err := s.seriesRepo.GetAll(ctx, filters)
	if err != nil {
