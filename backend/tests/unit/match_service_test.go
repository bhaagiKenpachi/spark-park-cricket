...
func TestMatchService_CreateMatch(t *testing.T) {
	tests := []struct {
		...
		{
			name: "successful match creation with team names from series",
			request: &models.CreateMatchRequest{
				SeriesID:         "series-1",
				MatchNumber:      intPtr(1),
				Date:             time.Date(2025, 9, 14, 10, 0, 0, 0, time.UTC),
			...
			mockSetup: func(matchRepo *MockMatchRepository, seriesRepo *MockSeriesRepository) {
				teamAName := "Team Lions"
				teamBName := "Team Tigers"
				seriesRepo.On("GetByID", mock.Anything, "series-1").Return(&models.Series{
					ID:        "series-1",
					TeamAName: &teamAName,
					TeamBName: &teamBName,
				}, nil)
				matchRepo.On("ExistsBySeriesAndMatchNumber", mock.Anything, "series-1", 1).Return(false, nil)
				matchRepo.On("Create", mock.Anything, mock.MatchedBy(func(match *models.Match) bool {
					// Verify team names are copied from series
					return match.TeamAName != nil && *match.TeamAName == "Team Lions" &&
						match.TeamBName != nil && *match.TeamBName == "Team Tigers"
