...
async function addInsightsToDashboard() {
    ...
    try {
        // First, let's find the Series Analytics dashboard
        console.log(chalk.blue('📋 Finding Series Analytics dashboard...'));
        const dashboardsResponse = await client.makeRequest('/dashboards/');

        const seriesDashboard = dashboardsResponse.results?.find(d =>
            d.name === 'Series Analytics' || d.name.includes('Series')
        );

        if (!seriesDashboard) {
            console.error(chalk.red('❌ Series Analytics dashboard not found.'));
            console.error(chalk.yellow('💡 Please create the dashboard first by running:'));
            console.error(chalk.yellow('   npm run posthog:dashboard:create'));
            return;
        }

        console.log(chalk.green(`✅ Found dashboard: ${seriesDashboard.name} (ID: ${seriesDashboard.id})`));
        console.log(chalk.cyan(`🔗 Dashboard URL: ${client.getDashboardUrl(seriesDashboard.id)}\n`));

        // Get current dashboard tiles
        const dashboard = await client.makeRequest(`/dashboards/${seriesDashboard.id}/`);
        ...
        for (const insight of SERIES_INSIGHT_IDS) {
            try {
                ...
                await client.makeRequest(`/dashboards/${seriesDashboard.id}/`, {
                    method: 'PATCH',
