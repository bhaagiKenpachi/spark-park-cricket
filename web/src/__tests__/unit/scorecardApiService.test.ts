...
import { ScorecardResponse, BallEventRequest, OverSummary, InningsSummary } from '../../store/reducers/scorecardSlice';
...
describe('ApiService - Scorecard Endpoints', () => {
    ...
    describe('getScorecard', () => {
        ...
        it('should handle API errors', async () => {
            ...
            await expect(apiService.getScorecard('nonexistent-match')).rejects.toThrow(ApiError);
        });

        it('should handle network errors', async () => {
            mockFetch.mockRejectedValueOnce(new Error('Network error'));

            await expect(apiService.getScorecard('match-1')).rejects.toThrow('Network error');
        });
    });
    ...
    describe('startScoring', () => {
        ...
        it('should handle start scoring errors', async () => {
            ...
            await expect(apiService.startScoring('match-1')).rejects.toThrow(ApiError);
        });
    });
    ...
    describe('getCurrentOver', () => {
        ...
        it('should handle get current over errors', async () => {
            ...
            await expect(apiService.getCurrentOver('match-1', 1)).rejects.toThrow(ApiError);
        });
    });
    ...
    describe('getInnings', () => {
        ...
        it('should handle get innings errors', async () => {
            ...
            await expect(apiService.getInnings('match-1', 1)).rejects.toThrow(ApiError);
        });
    });
    ...
    describe('getOver', () => {
        const mockOverResponse: OverSummary = {
            ...
            status: 'completed',
            balls: [
                { ball_number: 1, ball_type: 'good', run_type: '4', runs: 4, byes: 0, is_wicket: false },
                { ball_number: 2, ball_type: 'good', run_type: '2', runs: 2, byes: 0, is_wicket: false },
                { ball_number: 3, ball_type: 'good', run_type: 'WC', runs: 0, byes: 0, is_wicket: true, wicket_type: 'bowled' },
                { ball_number: 4, ball_type: 'good', run_type: '6', runs: 6, byes: 0, is_wicket: false },
            ],
        };
        ...
        it('should handle get over errors', async () => {
            ...
            await expect(apiService.getOver('match-1', 1, 3)).rejects.toThrow(ApiError);
        });
    });
