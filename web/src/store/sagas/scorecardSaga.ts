...
export function* fetchScorecardSaga(
  ...
  void,
  ApiResponse<{ data: ScorecardResponse }>
> {
  try {
    const apiService = new ApiService();
    const response = yield call(
      apiService.getScorecard.bind(apiService),
      action.payload
    );
    yield put(fetchScorecardSuccess(response.data.data || response.data));
  } catch (error) {
    const errorMessage =
      error instanceof ApiError ? error.message : 'Failed to fetch scorecard';
...
export function* fetchInningsSaga(
  ...
  CallEffect | PutEffect,
  void,
  ApiResponse<{ data: InningsSummary }>
> {
  try {
    const apiService = new ApiService();
    const { matchId, inningsNumber } = action.payload;
    const response = yield call(
      apiService.getInnings.bind(apiService),
      matchId,
      inningsNumber
    );
    ...
    const errorMessage =
      error instanceof ApiError ? error.message : 'Failed to fetch innings';
