...
export function* createSeriesSaga(
  action: ReturnType<typeof createSeriesRequest>
): Generator<CallEffect | PutEffect, void, ApiResponse<{ data: Series }>> {
  try {
    const apiService = new ApiService();
    const response = yield call(
      apiService.createSeries.bind(apiService),
      action.payload
    );
    yield put(createSeriesSuccess(response.data.data || response.data));
  } catch (error) {
    const errorMessage =
      error instanceof ApiError ? error.message : 'Failed to create series';
    yield put(createSeriesFailure(errorMessage));
  }
}

export function* updateSeriesSaga(
  action: ReturnType<typeof updateSeriesRequest>
): Generator<CallEffect | PutEffect, void, ApiResponse<{ data: Series }>> {
  try {
    const apiService = new ApiService();
    const { id, seriesData } = action.payload;
    const response = yield call(
      apiService.updateSeries.bind(apiService),
      id,
      seriesData
    );
    ...
    const errorMessage =
      error instanceof ApiError ? error.message : 'Failed to update series';
    yield put(updateSeriesFailure(errorMessage));
  }
}

export function* deleteSeriesSaga(
  action: ReturnType<typeof deleteSeriesRequest>
  ...
  try {
    ...
    const errorMessage =
      error instanceof ApiError ? error.message : 'Failed to delete series';
